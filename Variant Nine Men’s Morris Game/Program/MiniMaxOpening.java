...
public class MiniMaxOpening {
	...
	public ArrayList<char[]> generateMovesMidgameEndgame(char[] board) {
	...
	public static int minMax(char[] board, int depth, int goalDepth) {
		...
		} else { 						// if(depth%2==1)odd; black- min
			ArrayList<char[]> childList = generateAddB(board);
			v = 10000;
			for (int i = 0; i < childList.size(); i++) {
				board = childList.get(i);
				v = Math.min(v, minMax(board, depth + 1, goalDepth));
			}
		}
		return v;
	}
	...
	public ArrayList<char[]> generateHopping(char[] board) {
		ArrayList<char[]> list = new ArrayList<>();
		for (int i = 0; i < board.length; i++) {
			if (board[i] == 'W') {
	...
	public ArrayList<char[]> generateMove(char[] board) {
		ArrayList<char[]> list = new ArrayList<>();
		for (int i = 0; i < board.length; i++) {
			if (board[i] == 'W') {
	...
	public List<Integer> neighbors(int j) {
	...
	public static boolean closeMill(int j, char[] board) {
		...
		switch (j) {
			...
			case 13:
				if ((board[6] == C && board[9] == C) || (board[16] == C && board[19] == C) || (board[14] == C && board[15] == C)) {
					return true;
				} else {
					return false;
				}
			...
			case 15:
				if ((board[13] == C && board[14] == C) || (board[7] == C && board[10] == C) || (board[18] == C && board[21] == C)) {
					return true;
				} else {
					return false;
				}
			case 16:
				if ((board[13] == C && board[19] == C) || (board[3] == C && board[8] == C) || (board[17] == C && board[18] == C)) {
					return true;
				} else {
					return false;
				}
			...
			case 18:
				if ((board[16] == C && board[17] == C) || (board[15] == C && board[21] == C) || (board[5] == C && board[11] == C)) {
					return true;
				} else {
					return false;
				}
			...
			case 21:
				if ((board[19] == C && board[20] == C) || (board[18] == C && board[15] == C) || (board[12] == C && board[2] == C)) {
					return true;
				} else {
					return false;
				}
